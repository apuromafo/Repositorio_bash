...
if command -v gnome-terminal &>/dev/null; then
    ...
    # Fallback: usar bash directamente (puede no mostrar bien la salida)
    TERMINAL_EXEC="bash -c"
fi

# Nombre del script principal
SCRIPT_NAME="ssh_protection_audit.sh"
...
# Nombre del lanzador
DESKTOP_FILE_NAME="ssh_protection_audit.desktop"
DESKTOP_FILE_PATH="$APPS_DIR/$DESKTOP_FILE_NAME"

ICON_PATH="$HOME/.local/share/icons/ssh-audit-icon.png"
...
[Desktop Entry]
Name=SSH Protection Audit
Comment=Herramienta de auditoría SSH contra fuerza bruta
